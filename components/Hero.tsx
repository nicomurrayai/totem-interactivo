...
const kpis = [
  ...
  { value: "4K", label: "Resolucion ultra" },
...
export default function Hero() {
  return (
    <section className="pb-10 pt-8 sm:pb-16 sm:pt-12 lg:pb-20">
      <div className="grid items-center gap-10 lg:grid-cols-[1.08fr_0.92fr] lg:gap-8">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-[#bfdbff] bg-[#eaf3ff] px-4 py-2 text-xs font-bold uppercase tracking-[0.1em] text-[#4a8df1]">
            <span className="size-2 rounded-full bg-[#22c3ef]" aria-hidden />
            Nueva generacion 2026
          </div>

          <h1 className="mt-6 max-w-[18ch] text-[clamp(1.65rem,3.6vw,3rem)] font-bold leading-[1.2] tracking-[-0.04em] text-[#0d1733]">
            Totems interactivos para experiencias{" "}
            <span className="text-[#4f8dfd]">que impactan.</span>
          </h1>

          <p className="mt-6 max-w-[28ch] text-[clamp(0.95rem,1.4vw,1.25rem)] font-semibold leading-[1.22] text-[#364563]">
            Juegos, aplicaciones y desarrollos a medida para eventos y marcas.
          </p>

          <div className="mt-8 flex flex-wrap items-center gap-4">
            <Button asChild className="h-11 rounded-2xl bg-[#2f67ea] px-6 text-sm font-semibold text-white shadow-[0_14px_24px_rgba(47,103,234,0.34)] hover:bg-[#2259d8] sm:px-7 sm:text-base">
          ...
          <div className="mt-8 flex flex-col gap-5 text-[#1f2e4e] sm:flex-row sm:items-start sm:gap-0">
            {kpis.map((item, index) => (
              <div
                key={item.value}
                className={`sm:px-7 ${
              ...
              >
                <p className="text-[1.6rem] font-bold leading-none tracking-[-0.03em]">
                  {item.value}
                </p>
        ...
        <div className="mx-auto w-full max-w-[270px] lg:max-w-[290px]">
              ...
              <div className="rounded-[1.4rem] border-none  bg-gradient-to-br from-[#11264a] to-[#091733] p-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {totemCards.map(({ id, icon: Icon, label }) => (
                    <div
                      key={id}
                      className="rounded-[1.2rem] border border-[#3e5a89] bg-[#1b3159]/65 px-3 py-4 cursor-pointer hover:scale-105 transition-all"
                    >
